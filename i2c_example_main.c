...
void display_mf()
{
	...
	{
		count = count + 1;	
			if (count == 17)	//khi biến count = 17 tức là hàng ngang đã đủ 128 pixel nên chúng ta xuống hàng
			{
			cmd = i2c_cmd_link_create();
			i2c_master_start(cmd);
			i2c_master_write_byte(cmd, (OLED_I2C_ADDRESS << 1) | I2C_MASTER_WRITE, true);
			...
			i2c_master_cmd_begin(I2C_NUM_0, cmd, 10/portTICK_PERIOD_MS);
			i2c_cmd_link_delete(cmd);
			count = 0;	//đưa biến count về 0 để bắt đầu lại việc đếm
			i = i - 1;	//trừ biến i đi 1 giá trị để tránh mất dữ liệu
			}
			else
			{
			cmd = i2c_cmd_link_create();
			...
			i2c_master_write_byte(cmd, OLED_CONTROL_BYTE_DATA_STREAM, true);
			i2c_master_write(cmd, font8x8_basic_logo1[i], 8, true);
...
void app_main(void)
{
    ...
    ssd1306_init();
	//task_ssd1306_display_clear(text);
